 ...
 * CAN motor controller followers or {@link PWMMotorController#addFollower(PWMMotorController)}.
 *
 ...
 *
 * <p>Inputs smaller then {@value DriveBase#kDefaultDeadband} will be set to 0, and larger values
 * will be scaled so that the full range is still used. This deadband value can be changed with
 * {@link #setDeadband}.
 *
 * <p>{@link MotorSafety} is enabled by default. The tankDrive, arcadeDrive, or curvatureDrive
 * methods should be called periodically to avoid Motor Safety timeouts.
 */
public class DifferentialDrive extends DriveBase implements Sendable, AutoCloseable {
   ...
   * PWMMotorController#addFollower(PWMMotorController)}. If a motor needs to be inverted, do so
