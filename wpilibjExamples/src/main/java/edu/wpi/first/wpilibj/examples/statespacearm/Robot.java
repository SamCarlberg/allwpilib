...
public class Robot extends TimedRobot {
  ...
  private static final Measure<Angle> kRaisedPosition = Degrees.of(90.0);
  private static final Measure<Angle> kLoweredPosition = Degrees.of(0.0);
  ...
  // Nominal time between loops. 20ms for TimedRobot, but can be lower if using notifiers.
  ...
  private final LinearQuadraticRegulator<N2, N1, N1> m_controller =
      new LinearQuadraticRegulator<>(
          m_armPlant,
          VecBuilder.fill(Radians.convertFrom(1, Degrees), Radians.convertFrom(10, Degrees)),
          // qelms. Position and velocity error tolerances, in radians and radians per second.
  ...
  private final LinearSystemLoop<N2, N1, N1> m_loop =
      new LinearSystemLoop<>(m_armPlant, m_controller, m_observer, 12.0, kUpdatePeriod.in(Seconds));
  ...
  public void robotInit() {
    // We go 4096 counts in one rotation
    m_encoder.setDistancePerPulse(Rotations.one().divide(4096.0).in(Radians));
  }
  ...
  public void teleopPeriodic() {
    ...
    if (m_joystick.getTrigger()) {
      ...
      goal = new TrapezoidProfile.State(kRaisedPosition, RadiansPerSecond.zero());
    } else {
      // Otherwise, we go to the low goal
      goal = new TrapezoidProfile.State(kLoweredPosition, RadiansPerSecond.zero());
    }
