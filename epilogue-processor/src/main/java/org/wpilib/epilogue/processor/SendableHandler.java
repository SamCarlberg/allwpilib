...
public class SendableHandler extends ElementHandler {
  ...
  protected SendableHandler(ProcessingEnvironment processingEnv) {
    ...
    m_commandType =
        Optional.ofNullable(lookupTypeElement(processingEnv, "org.wpilib.commands2.Command"))
            .map(TypeElement::asType);
    m_subsystemType =
        Optional.ofNullable(lookupTypeElement(processingEnv, "org.wpilib.commands2.SubsystemBase"))
            .map(TypeElement::asType);
  }
